...
<div class="modal fade" tabindex="-1" id="edit{{ $data->id }}">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="" method="POST" id="submit-edit">
                @csrf
                <div class="modal-body pt-5">
                    <div class="form-group">
                        ...
                        <input type="text" name="category" class="form-control " placeholder="Input Category"
                            id="cat_id" value="{{ old('category') ?? $data->name }}">
                        <span class="text-danger error-message category""></span>
                    </div>
                </div>
                <input type="hidden" name="id" value="{{ $data->id }}">
                <div class="modal-footer">
                    <button type="button" class="btn btn-danger submit-edit" data-bs-dismiss="modal">Close</button>
...
<script>
    $('#submit-edit').on('submit', function(e) {
        e.preventDefault()
        $('.text-danger').html('')
        let cat_id = $(this).find('#cat_id').val()
        let url = "{{ route('admin.category.update', ':uuid') }}"
        url = url.replace(':uuid', cat_id);
        $.ajaxSetup({
            headers: {
